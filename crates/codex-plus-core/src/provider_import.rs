...
mod tests {
    ...
    fn parses_codexplusplus_provider_url() {
        let url = "codexplusplus://v1/import/provider?resource=provider&name=BRO%20API%20%E4%B8%AD%E8%BD%AC%E7%AB%99&baseUrl=https%3A%2F%2Fapi.skuzi.cn%2Fv1&apiKey=sk-test&wireApi=responses&relayMode=pureApi&configContents=bW9kZWxfcHJvdmlkZXIgPSAiQ29kZXhQbHVzUGx1cyIK&authContents=eyJPUEVOQUlfQVBJX0tFWSI6InNrLXRlc3QifQo%3D";

        let request = request_from_url(url).unwrap();

        assert_eq!(request.name, "BRO API 中转站");
        assert_eq!(request.base_url, "https://api.skuzi.cn/v1");
    ...
    fn pending_provider_import_round_trips_and_clears() {
        ...
        assert_eq!(pending.name, "BRO API 中转站");
        assert_eq!(pending.base_url, "https://api.skuzi.cn/v1");
